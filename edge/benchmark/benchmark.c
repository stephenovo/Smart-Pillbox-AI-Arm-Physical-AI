...
int main(int argc, char **argv) {
  ...
  careloop_features_t sample = {18,91,30,72,84,5};
  careloop_init();
  double int8_runs[7],fp32_runs[7];
  for (size_t i=0;i<10;i++) {checksum+=careloop_infer_int8(sample).missed_window;checksum+=careloop_infer_fp32(sample).missed_window;}
  for(size_t run=0;run<7;run++) {
    uint64_t start=ns();
    for(size_t i=0;i<iterations;i++)checksum+=careloop_infer_int8(sample).missed_window;
    int8_runs[run]=(double)(ns()-start)/(double)iterations;
    start=ns();
    for(size_t i=0;i<iterations;i++)checksum+=careloop_infer_fp32(sample).missed_window;
